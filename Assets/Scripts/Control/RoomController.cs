...
public class RoomController : MonoBehaviour {
	...
	public AtmospherePacket ideal;
	public AnimationCurve iFlowRate; //m^3/s (Pa)
	public AnimationCurve oFlowRate; //m^3/s (Pa)
	
	void Start() {
		atmosphere = GetComponent<Atmosphere>();
		ideal = new AtmospherePacket();
		Refresh();
	}
	
	public void Refresh() {
		ideal.Reset();
		ideal.volume = atmosphere.volume;
		ideal["Oxygen"] = o2;
		ideal["Oxygen"] = 1-o2;
		
		iFlowRate = new AnimationCurve(
			new Keyframe(0, 0),
			new Keyframe(pressure, nomFlowRate),
			new Keyframe(2*pressure, 2*nomFlowRate)
			);
		oFlowRate = new AnimationCurve(
			new Keyframe(0, 2*nomFlowRate),
			new Keyframe(pressure, nomFlowRate),
			new Keyframe(2*pressure, 0)
			);
	}
}
